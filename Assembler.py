...
# function to write the corresponding binary value of instructions
def type_ins(instruction):
    # spliting instruction
    instruction=split_ins(instruction)
...
bonus = {"halt","rst","mul","rvrs"}

file = open(file_name, "r")
all_instructions = file.readlines()
c = -1 # line nos. are being started from 0
output = ""
for line in all_instructions:
    ...
    cleaned_line = cleaned_line.rstrip()
    if cleaned_line: #taking care of empty line    
        output += type_ins(cleaned_line) + "\n"

file.close()

# Writing the output to a file named output.txt
